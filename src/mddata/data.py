...
class UpdateOperation:
    ...
    @property
    def section_policy(self) -> UpdatePolicy:
        """Get policy for the operation."""
        if self.policy is not None:
            return self.policy
        return self._get_policy(self.data)

    def _get_policy(self, value: InputDataOptions) -> UpdatePolicy:
...
class MarkdownData:
    ...
    def _validate_changes(self) -> None:
        ...
        if self._validator:
            ...
            if not result["valid"]:
                validation_level = self._validator.validation_level
        ...
        if "sections" in changes:
            for section_update in changes["sections"]:
                ...
                try:
                    ...
                    elif policy == UpdatePolicy.REPLACE:
