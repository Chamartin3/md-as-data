...
The command-line interface provides quick access to querying, modifying, and exporting markdown documents.
...
```bash
# Get document sections and structure
uv run mdasdata document.md info sections

# Get document properties (frontmatter)
uv run mdasdata document.md info properties

# Get document blocks with filtering
uv run mdasdata document.md info blocks --type paragraph
```

**Example Sections Output:**
```
document.md sections:
├── introduction (H1) - 3 blocks
│   └── overview (H2) - 2 blocks
├── methodology (H1) - 4 blocks
│   ├── data_collection (H2) - 2 blocks
│   └── analysis (H2) - 3 blocks
└── conclusion (H1) - 1 blocks
```

**Example Properties Output:**
```
document.md frontmatter properties:
• title: "My Document"
• author: "John Doe"
• date: "2024-01-15"
• tags: ["example", "documentation"]
