...
|---------|---------|---------|
| `mddata schema infer` | Create schema from document(s) | `mddata schema infer doc.md -o schema.json` |
...
| `mddata write --schema` | Generate template from schema | `mddata write --schema schema.json -o template.md` |
...
# YAML format
mddata schema infer document.md --format yaml --output schema.yaml

# From multiple documents (aggregated)
mddata schema infer ./docs/ --output aggregated_schema.json --pretty
```
...
mddata schema infer example.md --output standard_schema.json

# 2. Create template from schema
mddata write --schema standard_schema.json --output template.md
...
mddata schema infer ./docs/ --output docs_schema.json
